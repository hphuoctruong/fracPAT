...
class Prior:
    ...
    - :code:`Msolver`: an operator to apply the inverse of the mass matrix.
...
class SqrtPrecisionPDE_Prior(Prior):
    """
    ...
    def __init__(self, Vh, sqrt_precision_varf_handler, mean=None, rel_tol=1e-12, max_iter=1000):
        ...
        old_qr = dl.parameters["form_compiler"]["quadrature_degree"]
        dl.parameters["form_compiler"]["quadrature_degree"] = -1
        q_degree = 2 * Vh._ufl_element.degree()
        ...
        if num_sub_spaces <= 1: #SCALAR PARAMETER
            element = ufl.FiniteElement("Quadrature", Vh.mesh().ufl_cell(), q_degree, quad_scheme="default")
        else: #Vector FIELD PARAMETER
            element = ufl.VectorElement("Quadrature", Vh.mesh().ufl_cell(),
                                       q_degree, dim=num_sub_spaces, quad_scheme="default")
        Qh = dl.FunctionSpace(Vh.mesh(), element)
...
def BiLaplacianPrior(Vh, gamma, delta, Theta=None, mean=None, rel_tol=1e-12, max_iter=1000, robin_bc=False):
    ...
    def sqrt_precision_varf_handler(trial, test): 
        if Theta is None:
            varf_L = gamma * ufl.inner(ufl.grad(trial), ufl.grad(test)) * ufl.dx
        else:
            varf_L = gamma * ufl.inner(Theta*ufl.grad(trial), ufl.grad(test)) * ufl.dx
        ...
        return varf_L + varfM + varf_robin
...
class GaussianRealPrior(Prior):
    ...
    def __init__(self, Vh, covariance, mean=None):
        ...
        self.chol_inv = scila.solve_triangular(self.chol, np.identity(Vh.dim()), lower=True)
        ...
        var_form_R = domain_measure_inv \
                     * ufl.inner(test, precision_op * trial) * ufl.dx

        # Variational for the inverse regularization operator, or the covariance matrix
        var_form_Rinv = domain_measure_inv \
                        * ufl.inner(test, covariance_op * trial) * ufl.dx

        # Variational form for the square root of the regularization operator
        var_form_R_sqrt = domain_measure_inv \
                          * ufl.inner(test, chol_inv_op.T * trial) * ufl.dx

        # Variational form for the square root of the inverse regularization operator
        var_form_Rinv_sqrt = domain_measure_inv \
                             * ufl.inner(test, chol_op * trial) * ufl.dx
