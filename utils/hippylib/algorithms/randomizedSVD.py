...
[1] Nathan Halko, Per Gunnar Martinsson, and Joel A. Tropp,
Finding structure with randomness:
Probabilistic algorithms for constructing approximate matrix decompositions,
SIAM Review, 53 (2011), pp. 217-288.

[2] Per Gunnar Martinsson
...
def accuracyEnhancedSVD(A, Omega, k, s=1, check=False):
    ...
    - :code:`Omega`: a random Gaussian matrix with :math:`m \\geq k` columns.
    ...
    n_vec = Omega.nvec()
    assert n_vec >= k, 'Omega must have at least k columns.'

    Z = MultiVector(Omega)

    y_vec = fn.Vector(A.mpi_comm())
    A.init_vector(y_vec, 0)
    Y = MultiVector(y_vec, n_vec)
    MatMvMult(A, Omega, Y)
    ...
    # Form B^T = A^TQ (B = Q^TA) and orthogonalize in one step
