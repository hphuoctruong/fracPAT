...
def singlePass(A, Omega: MultiVector, k: int, s=1, check=False):
    """
    ...
    """
    n_vec = Omega.nvec()
    assert n_vec >= k, "Number of columns in Omega must be greater than k."
    ...
    Z_tranpose = Y_pr.dot_mv(Q)
    Wt = Y.dot_mv(Q)
        
    Tt = np.linalg.solve(Z_tranpose, Wt)
