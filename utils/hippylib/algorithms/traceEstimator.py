...
class TraceEstimator:
    ...
    """
    def __init__(self, A, solve_mode=False, accuracy=1e-1, init_vector=None, random_engine=rademacher_engine, mpi_comm=dl.MPI.comm_world):
        """
        Constructor:

        - :code:`A`:             an operator
        - :code:`solve_mode`:    if :code:`True` we estimate the trace of :code:`A`:math:`^{-1}`, otherwise of :code:`A`.
        - code:`accuracy`:     we stop when the standard deviation of the estimator is less then :code:`accuracy`*tr(:code:`A`).
        - :code:`init_vector`:   use a custom function to initialize a vector compatible with the range/domain of :code:`A`.
    ...
    def __call__(self, min_iter=5, max_iter=100):
        ...
        self.converged = True
        while math.sqrt(var_tr) > self.accuracy*exp_tr:
            self.iter += 1
            self.random_engine(self.z)
            self.A.mult(self.z, self.Az)
            tr = self.z.inner(self.Az)
