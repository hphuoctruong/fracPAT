...
class LowRankOperatorNonSymmetric:
    def __init__(self, u, Sigma, v, my_init_vector=None):
        self.u = u
        self.Sigma = Sigma
        self.v = v
...
class LowRankOperator:
    ...
    def trace(self, W=None):
        ...
        if W is None:
            temp = self.U[0].copy()
            temp.zero()
            self.U.reduce(temp, np.sqrt(self.d))
            tr = temp.inner(temp)
        else:
